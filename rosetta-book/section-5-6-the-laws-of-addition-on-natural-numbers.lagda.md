...
<!-- rosetta-item: proposition-5.6.1; latex-label: prp:unit-laws-add-N -->
...
<!-- rosetta-item: subheading-5.6-proof -->
...
succ-ℕ(0+n)=succ-ℕ(n),
```
because by the computation rule for addition we have that `0+succ-ℕ(n)≐succ-ℕ(0+n)`.
...
<!-- rosetta-agda-block: proposition-5.6.1-unit-laws-addition-natural-numbers -->
...
<!-- rosetta-item: proposition-5.6.2; latex-label: prp:successor-laws-add-N -->
...
<!-- rosetta-item: subheading-5.6-proof-2 -->
...
<!-- rosetta-agda-block: proposition-5.6.2-successor-laws-addition-natural-numbers -->
...
<!-- rosetta-item: proposition-5.6.3 -->
...
<!-- rosetta-item: subheading-5.6-proof-3 -->

*Proof.* We construct `associative-add-ℕ(m,n,k)` by induction on `k`.
...
For the inductive step, let `p:(m+n)+k=m+(n+k)`.
Our goal is to show that
```text
...
<!-- rosetta-agda-block: proposition-5.6.3-associativity-addition-natural-numbers -->
...
<!-- rosetta-item: proposition-5.6.4 -->
...
<!-- rosetta-item: subheading-5.6-proof-4 -->

*Proof.* We construct `commutative-add-ℕ(m,n)` by induction on `m`.
In the base case we have to show that `0+n=n+0`, which holds by the unit laws for `n`, proven in Proposition 5.6.1.

For the inductive step, let `p:m+n=n+m`.
Our goal is to construct an identification `succ-ℕ(m)+n=n+succ-ℕ(m)`.
...
≐ n+succ-ℕ(m).
```
The first identification is obtained by Proposition 5.6.2, and the second identification is the identification `ap_{succ-ℕ}(p)`. ◻

<!-- rosetta-agda-block: proposition-5.6.4-commutativity-addition-natural-numbers -->
