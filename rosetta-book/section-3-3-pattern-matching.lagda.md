...
<!-- rosetta-item: section-3.3 -->

Note that in definition Definition 3.2.1 we stated that `add-ℕ` is a function of type `ℕ→ (ℕ→ℕ)` satisfying the specification
```text
add-ℕ(m,0) ≐ m
...
```text
\addpN(0,0) ≔ 0
\addpN(0,succ-ℕ(n)) ≔ succ-ℕ(n)
\addpN(succ-ℕ(m),0) ≔ succ-ℕ(m)
\addpN(succ-ℕ(m),succ-ℕ(n)) ≔ succ-ℕ(succ-ℕ(\addpN(m,n)).
```
...
<!-- rosetta-agda-block: section-3-3-pattern-matching-block-74 -->
