...
It turns out that if we naively define a function `f` to be an equivalence if it has an inverse, then we won’t be able to show that `is-equiv(f)` is a property.
We will therefore say that `f` is an equivalence if it has a separate left and right inverse.
This may look odd, but when we define equivalences in this way we will be able to show that `is-equiv(f)` is a property.
