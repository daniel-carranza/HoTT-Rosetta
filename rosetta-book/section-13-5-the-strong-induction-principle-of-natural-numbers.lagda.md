...
```agda
module section-13-5-the-strong-induction-principle-of-natural-numbers where
```
...
```text
s̃(n+1,m,p)=p̃_S(n,s̃(n),m,p)=\begin{cases}s̃(n,m,p) if m≤ n
p_S(n,s̃(n)) if m=n+1.\end{cases}
```
