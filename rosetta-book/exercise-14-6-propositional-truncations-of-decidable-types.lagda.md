...
Define a function `f:A→ Σ(x:A) inl(x)=d` and show that `f` is a propositional truncation of `A`.
...
π(inr(x)) := 0.
```
Define a function `g:A→ (π(d)=1)` and show that `g` is a propositional truncation of `A`.
