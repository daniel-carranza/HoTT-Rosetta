...
```text
binom_𝒰(A, B)≃ \sum_{(P:A→decidable-Prop_𝒰)}‖B≃Σ(a:A) P(a)‖.
```
from the binomial type `binom_𝒰(A, B)` to the type of decidable subtypes of `A` that are merely equivalent to `B`.
...
```text
binom_𝒰(A, B)≃ \sum_{(f:A→bool)}‖B≃Σ(a:A) f(a)=true‖.
```
...
binom(empty, B+unit) ≃ empty binom(A+unit, B+unit) ≃ binom(A, B)+binom(A, B+unit).
```
...
```
The type of decidable embeddings `empty↪ᵈ X` is contractible, because the type `empty→ X` is contractible with the map `ex-falso:empty→ X` at the center of contraction, which is of course a decidable embedding.

Next, the fact that the binomial type `binom(empty, B+unit)` is empty follows from the fact that the type of maps `X→empty` is empty for any type `X` merely equivalent to `B+unit`.

For the last equivalence we will use Proposition 17.6.6.
Using the universal property of `A+unit`, we see that
```text
binom(A+unit, B+unit)≃ Σ(P:A→decidable-Prop_𝒰) Σ(Q:decidable-Prop_𝒰) ‖(B+unit)≃ Σ(a:A) P(a)+ Q‖.
```
Using the fact that `decidable-Prop_𝒰≃Fin{2}`, observe that we have an equivalence
```text
Σ(Q:decidable-Prop_𝒰) ‖(B+unit)≃(Σ(a:A) P(a)+Q)‖
≃ ‖(B+unit)≃(Σ(a:A) P(a)+unit)‖+‖(B+unit)≃Σ(a:A) P(a)‖.
```
Furthermore, note that it follows from Proposition 16.2.1 that
```text
‖(B+unit)≃(Σ(a:A) P(a)+unit)‖≃‖B≃Σ(a:A) P(a)‖.
```
Thus we see that
```text
binom(A+unit, B+unit) ≃ (Σ(P:A→decidable-Prop_𝒰) ‖B≃Σ(a:A) P(a)‖)
