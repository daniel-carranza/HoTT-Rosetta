...
The review index explains where reviews are saved and how to share them directly
after its introductory sentence, using normal text styling. Other pages do not
repeat this tip. Git status stays hidden when the checkout is ready for reviews and
...
The notice includes collapsed lists you can click to expand or collapse:

- Uncommitted review changes: staged and unstaged changes listed separately,
  identifying each review item and whether its decision, comments, or evidence
  changed. A review can appear in both groups when it has further edits after
  staging. This also includes new or deleted review records and files.
- Committed but not pushed: local-only commit IDs and subjects, changed files,
  and individual review changes against each commit's first parent. Incoming-only
  commits are excluded even when local and remote main have diverged. This is a
  per-commit history, so an edit and its later reversal can both appear.

Recognized Agda review items link to their current review pages; unavailable
historical items and diagram reviews show their identifiers. The lists only read
Git and review files; they never stage, commit, push, or change anything. Unmerged
or unreadable files show an explanation instead of misleading item details.
